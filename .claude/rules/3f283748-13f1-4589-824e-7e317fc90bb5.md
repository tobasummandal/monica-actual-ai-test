<rule_activation id="3f283748-13f1-4589-824e-7e317fc90bb5" title="Adopt Inertia.js for Server-Driven Single Page Application Rendering: Controllers Use Inertia" applies_to="**/*">
These rules are ACTIVE for all web controllers in the Vault/ManageJournals domain and SHOULD be applied to new controller implementations across the application.
</rule_activation>

### Rules

- **R-INERTIA-001** SHOULD: Controllers SHOULD use Inertia's lazy loading for expensive data that is not immediately needed on page load.
...
<enforcement>
Claude Code MUST verify Inertia.js adoption through automated CI pipeline checks using grep patterns, code review checklists, PHPStan custom rules, and integration tests. Violations trigger CI pipeline failures and pull request blocks. Exceptions require documentation and tech lead approval.
</enforcement>
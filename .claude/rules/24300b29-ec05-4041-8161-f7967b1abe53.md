<rule_activation id="24300b29-ec05-4041-8161-f7967b1abe53" title="Adopt Mock-Based Unit Testing for External Service Dependencies in CI/CD Pipeline: Unit Tests Services" applies_to="tests/**/*Test.php">
These rules are ALWAYS ACTIVE for all unit test files in the codebase that interact with external services, APIs, or network resources.
</rule_activation>
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All unit tests interacting with external services MUST use mocks. Code review MUST verify mocking compliance before merge. CI/CD pipeline MUST fail if unit tests exceed 30-second execution threshold or show network dependency patterns.
</enforcement>
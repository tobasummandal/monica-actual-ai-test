...
These rules are ALWAYS ACTIVE for all PHP files. Password hashing is a critical security requirement that must be applied uniformly across all authentication entry points.
</rule_activation>

### Rules

- **R-PH-001** MUST: Password hashing MUST occur before persisting user credentials to the database in all authentication workflows
...
- Use `Hash::make($password)` for all password hashing operations
...
```bash
# Check for unhashed password assignments (should find none)
grep -r "'password'\s*=>\s*\$" app/ --include='*.php' | grep -v 'Hash::make' | grep -v 'bcrypt(' | grep -v '// ' || echo 'No unhashed password assignments found'

# Count password hashing implementations in authentication code
...
- Verification commands show no unhashed password assignments in authentication code

<enforcement>
Claude Code MUST NOT skip or defer verification. Password hashing is a non-negotiable security requirement.

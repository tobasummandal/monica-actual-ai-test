...
1. MUST: Session state for 2FA challenges MUST be stored using $request->session()->put() with login.id and login.remember keys before redirecting to challenge routes.

## Policy Block

- MUST Session state for 2FA challenges MUST be stored using $request->session()->put() with login.id and login.remember keys before redirecting to challenge routes.

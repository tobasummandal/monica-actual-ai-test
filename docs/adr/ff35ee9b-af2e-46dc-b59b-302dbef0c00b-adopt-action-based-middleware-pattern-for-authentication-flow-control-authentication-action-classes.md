...
1. MUST: Authentication action classes MUST implement a handle(Request $request, callable $next) method signature to participate in middleware-style request processing pipelines.

## Policy Block

- MUST Authentication action classes MUST implement a handle(Request $request, callable $next) method signature to participate in middleware-style request processing pipelines.

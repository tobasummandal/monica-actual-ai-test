...
1. MUST: Authentication action classes MUST call $this->limiter->increment($request) immediately before throwing authentication failure exceptions

## Policy Block

- MUST Authentication action classes MUST call $this->limiter->increment($request) immediately before throwing authentication failure exceptions

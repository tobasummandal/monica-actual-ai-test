...
1. MAY: Authentication actions MAY return early responses (redirects, JSON) for 2FA challenges instead of calling $next($request) to short-circuit the middleware pipeline.

## Policy Block

- MAY Authentication actions MAY return early responses (redirects, JSON) for 2FA challenges instead of calling $next($request) to short-circuit the middleware pipeline.

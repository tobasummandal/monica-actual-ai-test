...
1. MUST: Email field validation MUST include Rule::unique() with ignore() for the current user ID to prevent duplicate email addresses while allowing profile updates

## Policy Block

- MUST Email field validation MUST include Rule::unique() with ignore() for the current user ID to prevent duplicate email addresses while allowing profile updates
